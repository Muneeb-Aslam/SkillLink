...
const Skills: React.FC<{ data: any }> = ({ data }) => {
   ...
   return (
      <section className="py-8 px-16 w-full flex justify-between items-start gap-4 ">
         <form
            className="p-8 h-max relative w-3/5 border-[1.5px] border-primary flex flex-col justify-between items-start gap-4"
            onSubmit={handleSubmit(onSubmit)}
         >
            <div className="w-full flex justify-between items-center">
               <h3 className="text-blackish font-bold text-xl">Skills</h3>
               <div className="border-[1.5px] border-primary w-8 h-8 bg-white cursor-pointer flex justify-start items-center gap-2">
                  {!!edit && (
                     <Image
                        src={add}
                        alt="add"
